...
weight_save_path = os.path.join(model_path, 'weights/nl2sql_finetune_add_div.weights')
    ...
    def __iter__(self):
        while True:
            ...
            for i in idxs:
                ...
                CSEL0.append(csel[0]) # 条件中的列
                CSEL1.append(csel[1]) # 条件中的列
                CSEL2.append(csel[2]) # 条件中的列
...
pcsel = Dense(csel_num, activation='softmax')(pcsel) # [bs, q_step, 3, num_op]

pcsel0 = K.squeeze(K.slice(pcsel, [0, 0, 0, 0], [-1, -1, 1, -1]), axis=2)
pcsel1 = K.squeeze(K.slice(pcsel, [0, 0, 1, 0], [-1, -1, 1, -1]), axis=2)
pcsel2 = K.squeeze(K.slice(pcsel, [0, 0, 2, 0], [-1, -1, 1, -1]), axis=2)
...
model = Model(
    ...
    [psel, pconn, pcop, pcsel, pcdiv]  # outputs 
)
...
def nl2sql(question, table):
    ...
    if question in set(['哪种类型的房产可售去化时间低于36个月或者可售环比小于0',
    ...
    psel, pconn, pcop, pcsel, pcdiv = model.predict([
        np.array([x1]),
        np.array([x2]),
        np.array([h]),
        np.array([hm])
            ...
            if v_op != j:
                if v_op != -1:
                    ...
                    if max(pcdiv[0][v_start: v_end].argmax(1)) > 0:# 0 
                        ...
                        if max(pcsel1[0][v_start: v_end].argmax(1)) > 0:  # add 
    ...
    for i, j, k in conds:
        ...
        if j == 2: # 这里判定出条件运算符是等号哦  等号也有可能是数字的
            if k not in table['all_values']:
                k = most_similar_out(k, list(table['all_values'])) 
        ...
        idx += 1 
        if k.isnumeric() and i <= len(table['headers']) - 1:
            # 添加一个预处理，预测的数字左右缺失的话 
            ori_k = k 
            try:
